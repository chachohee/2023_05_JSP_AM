		...
		try {
			...
			if (article.isEmpty()) {
				...
				response.getWriter().append(
						String.format("<script>alert('%d번 게시물은 존재하지 않습니다.'); location.replace('list');</script>", id));
				return;
			}
			...
			response.getWriter()
					.append(String.format("<script>alert('%d번 게시물 삭제 성공'); location.replace('list');</script>", id));
